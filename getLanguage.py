...
def readConfig():
    ...
    # get language names by default language 1 is URDU and language 2 is ENGLISH
    lang1 = config_gen["language_1"].upper(
    ) if config_gen["language_1"] else "URDU"
    ...
    lang_1dict_names = config_dicts["language_1_dicts"].split(
        ",") if config_dicts["language_1_dicts"] else "urduct1"
...
def dictTagging(word, tag):
    ...
    if word.lower() in language_1_dicts["urduct1"].keys():
        dhin = 1
